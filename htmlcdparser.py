...
class HTMLCDParser(HTMLParser):
    def __init__(self):
        ...
        #Set User-Agent fix download cloudflare
        urllib.URLopener.version = 'Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.153 Safari/537.36 SE 2.X MetaSr 1.0'

    def handle_starttag(self, tag, attrs):
        #If we are in a non-nestable tag we do nothing        
        if self.inside_tag and not (self.inside_tag == "a" and tag == "img" and self.a_href) and not(self.inside_tag == "th" or self.inside_tag == "td" or self.inside_tag == "dt" or self.inside_tag == "dd"):         
            return
        if tag == "blockquote":
        ...
        if tag == "a" and self.inside_tag and ((self.inside_tag <> "pre" and self.inside_tag <> "code") and not self.inside_p and not self.inside_li and not self.inside_dl):   
            self.del_tag = self.inside_tag
            self.zim_str = self.zim_str[:len(self.zim_str)-len(self.start_tag)]            
        #Initialize non-nestable tag
        if  tag <> "td" and self.beg.get(tag) or tag == "a" and not self.inside_tag:
            self.inside_tag = tag
        ...
        if tag == "p":
            ...
            if src <> "#" and not self.inside_table:
                ...
                if not self.folder.exists():
                    self.folder.touch()                
                urllib.urlretrieve(src, self.folder.path + "/" + img_name)
    ...
    def handle_endtag(self, tag):        
        if self.inside_tag and tag <> self.inside_tag and not (self.inside_tag == "th" or self.inside_tag == "td" or self.inside_tag == "dt" or self.inside_tag == "dd" ):
            return
        if tag == "blockquote":
            self.inside_blockquote = False
        #end of nestable tag
        ...
        if self.del_tag == tag:                
            ...
            self.del_tag = ""
            return
        if (tag == "pre" or tag == "code"): #If pre not in p                                   
        ...
        if tag in self.end.keys():            
            ...
            if tag == "li" and not self.zim_str.endswith("\n"):
                self.zim_str += self.end[tag] + "\n"
            else:                
                #If we are not at a level higher than the first level of a list
